...
```bash
bash scripts/bench_prog3_task_counts.sh
```

The script temporarily edits `prog3_mandelbrot_ispc/mandelbrot.ispc`, rebuilds
Program 3, runs `./mandelbrot_ispc --tasks`, and restores the original ISPC
file on exit. Each task count uses 5 measured invocations and a 10 second
cooldown. Each invocation still uses Program 3's built-in minimum of 3 timing
repetitions.
...
- `means.csv`: mean values grouped by task count.
