#!/usr/bin/env bash
# Benchmark Program 4 with alternating fast and slow inputs for SIMD divergence.
...
OUT_DIR="$ROOT_DIR/artifacts/experiments/prog4/q3_alternating_fast_slow"
...
new = "values[i] = (i % 2 == 0) ? 1.0f : 2.999f;"
if old not in text:
...
{
  echo "=== Program 4 Q3 alternating fast/slow input: cooldown ${COOLDOWN}s + ${RUNS} measured invocations ==="
  echo "Input: values[i] = (i % 2 == 0) ? 1.0f : 2.999f"
  echo "Expected effect: each 8-wide gang mixes 0-iteration lanes with slow 2.999f lanes."
} | tee -a "$RAW_LOG"
