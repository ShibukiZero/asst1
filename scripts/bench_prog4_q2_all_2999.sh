#!/usr/bin/env bash
# Benchmark Program 4 with every input set to the slow in-range value 2.999f.
...
OUT_DIR="$ROOT_DIR/artifacts/experiments/prog4/q2_all_2999"
...
{
  echo "=== Program 4 Q2 all-2.999 input: cooldown ${COOLDOWN}s + ${RUNS} measured invocations ==="
  echo "Input: every value is 2.999f, the slowest checked in-range constant near the upper bound."
} | tee -a "$RAW_LOG"
