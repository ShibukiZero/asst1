#!/usr/bin/env bash
# Benchmark Program 5 saxpy with manual AVX2 streaming stores.
...
OUT_DIR="$ROOT_DIR/artifacts/experiments/prog5/q3_streaming"
...
printf "run,ispc_ms,ispc_gbps,ispc_gflops,task_ms,task_gbps,task_gflops,stream_ms,stream_gbps,stream_gflops,task_speedup,stream_speedup\n" > "$SUMMARY"
: > "$RAW_LOG"

{
  echo "=== Program 5 Q3 streaming-store saxpy: cooldown ${COOLDOWN}s + ${RUNS} measured invocations ==="
...
for run in $(seq 1 "$RUNS"); do
  ...
  stream_ms=$(echo "$out" | awk -F'[][]' '/saxpy streaming/ {print $4; exit}')
  stream_gbps=$(echo "$out" | awk -F'[][]' '/saxpy streaming/ {print $6; exit}')
  stream_gflops=$(echo "$out" | awk -F'[][]' '/saxpy streaming/ {print $8; exit}')
  task_speedup=$(echo "$out" | awk -F'[()]' '/speedup from use of tasks/ {gsub(/x speedup from use of tasks/, "", $2); print $2; exit}')
  stream_speedup=$(echo "$out" | awk -F'[()]' '/speedup from streaming stores vs ISPC/ {gsub(/x speedup from streaming stores vs ISPC/, "", $2); print $2; exit}')

  printf "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" \
    "$run" "$ispc_ms" "$ispc_gbps" "$ispc_gflops" \
    "$task_ms" "$task_gbps" "$task_gflops" \
    "$stream_ms" "$stream_gbps" "$stream_gflops" \
    "$task_speedup" "$stream_speedup" >> "$SUMMARY"
done
...
with open(dest, "w", newline="") as f:
    ...
    writer.writerow({
        ...
        "mean_stream_ms": mean("stream_ms"),
        "mean_stream_gbps": mean("stream_gbps"),
        "mean_stream_gflops": mean("stream_gflops"),
        "mean_task_speedup": mean("task_speedup"),
        "mean_stream_speedup": mean("stream_speedup"),
    })
PY
